...
--apparently they formated the date to include the canNum after a '#' thus looking like '%m/%d/%Y#CanNum'
UPDATE Record R
SET R.RealDate =  STR_TO_DATE(SUBSTRING_INDEX(R.Date,'#',1),'%m/%d/%Y')

--this takes the CanNum part of the date if it exists and adds it to the corresponding column
--to run this, copy it into the phpmyadmin server
UPDATE Record R
SET R.CanNum =  SUBSTRING_INDEX(SUBSTRING_INDEX(R.Date,'#',2), '#', -1)
